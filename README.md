...
Assume we want to create a database, namely 'movies'. Then, we switch to 'movies' database.
```
...
```
And the second constraint as follows:
```
...
- Find the actor name "Tom Hanks":
```
MATCH (tom:Person {name: "Tom Hanks"}) RETURN tom
```
- Find the movie with title "Top Gun":
```
MATCH (topGun:Movie {title: "Top Gun"}) RETURN topGun
```
...
```
- Who directed "Top Guns"?
```
MATCH (cloudAtlas:Movie {title: "Top Guns"})<-[:DIRECTED]-(directors) RETURN directors.name
```
- Who were Tom Hanks' co-actors?
```
MATCH (tom:Person {name:"Tom Hanks"})-[:ACTED_IN]->(m)<-[:ACTED_IN]-(coActors) RETURN DISTINCT coActors.name
```
- How people are related to "Top Gun"?
```
MATCH (people:Person)-[relatedTo]-(:Movie {title: "Top Guns"}) RETURN people.name, Type(relatedTo), relatedTo.roles
```
- Degree: Use variable length patterns to find movies and actors up to 4 "hops" away from Kevin Bacon.
```
